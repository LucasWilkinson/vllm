...
@pytest.mark.parametrize(
    ("dcp_size", "tp_size"), [(1, 1), (8, 1), (8, 2)]
)
def test_dspark_pcp_accepted_without_peer_kv(dcp_size, tp_size):
    # The DSpark context-KV precompute all-gathers the PCP-sharded target
    # states and writes the draft rows each rank owns, so no PCP layout needs
    # symmetric-memory peer views.
    PCPManager.validate_config(
...
@pytest.mark.parametrize(
    ("pcp_size", "dcp_size", "expected"), [(4, 1, False), (4, 4, True), (4, 2, False)]
)
def test_dcp_spans_pcp(pcp_size, dcp_size, expected):
    manager = PCPManager(
    ...
    assert manager.dcp_spans_pcp is expected
