...
class Indexer(nn.Module):
    def __init__(
        ...
        self.indexer_side_stream = get_side_stream()
    ...
    def forward(
        ...
        if self.use_indexer_side_stream:
            assert self.indexer_side_stream is not None
            self.indexer_side_stream.wait_stream(torch.accelerator.current_stream())
            with self.indexer_side_stream:
